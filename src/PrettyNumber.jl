...
mantissa(x::Number) = x / exp10(exponent(x))
...
"""
Pretty print in scientific notation, with `l` decimal places. if `hideexp`,
the exponent will not be shown if it's 0.
Returns a `String`.
"""
...
1.420 has 4, for example, and 0.0540 has 3.
"""
...
"""
function prettify(str::AbstractString)
...
prettyprint(str) = str |> prettify |> println
